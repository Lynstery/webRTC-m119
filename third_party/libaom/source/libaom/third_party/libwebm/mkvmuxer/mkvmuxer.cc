...
uint32_t SeekHead::GetId(int index) const {
  if (index < 0 || index >= kSeekEntryCount) return UINT32_MAX;
  return seek_entry_id_[index];
}

uint64_t SeekHead::GetPosition(int index) const {
  if (index < 0 || index >= kSeekEntryCount) return UINT64_MAX;
...
SegmentInfo::SegmentInfo()
    : duration_(-1.0), muxing_app_(NULL), timecode_scale_(1000000ULL),
      writing_app_(NULL), date_utc_(INT64_MIN), duration_pos_(-1) {}
...
bool SegmentInfo::Write(IMkvWriter* writer) {
  ...
  if (date_utc_ != INT64_MIN) size += EbmlDateElementSize(libwebm::kMkvDateUTC);
