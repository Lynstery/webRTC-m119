...
void GtkMainWnd::OnRedraw() {
  ...
  if (remote_renderer && remote_renderer->image() != NULL &&
    ...
    // 按原始分辨率分配缓冲和设置绘制区域
    if (!draw_buffer_.get()) {
      draw_buffer_size_ = width_ * height_ * 4;  // ARGB 每像素 4 字节
      draw_buffer_.reset(new uint8_t[draw_buffer_size_]);
      gtk_widget_set_size_request(draw_area_, width_, height_);
    }

    // 远端帧直接拷贝（逐行 memcpy），不再做 2× 放大
    const uint8_t* src_bytes =
        reinterpret_cast<const uint8_t*>(remote_renderer->image());
    uint8_t* dst_bytes = draw_buffer_.get();
    const int dst_stride_bytes = width_ * 4;
    for (int r = 0; r < height_; ++r) {
      memcpy(dst_bytes + r * dst_stride_bytes,
             src_bytes + r * dst_stride_bytes,
             dst_stride_bytes);
    }
    // 叠加本地预览（右下角），做一个 1/4 缩小的缩略图
    VideoRenderer* local_renderer = local_renderer_.get();
    if (local_renderer && local_renderer->image()) {
      const uint32_t* lsrc =
          reinterpret_cast<const uint32_t*>(local_renderer->image());
      const int lw = local_renderer->width();
      const int lh = local_renderer->height();

      // 目标缩略图尺寸（1/4 缩小）
      const int thumb_w = std::max(1, lw / 4);
      const int thumb_h = std::max(1, lh / 4);

      // 放在右下角，保留 5px 边距
      const int margin = 5;
      const int dst_x = std::max(0, width_  - thumb_w - margin);
      const int dst_y = std::max(0, height_ - thumb_h - margin);

      uint32_t* dst32 = reinterpret_cast<uint32_t*>(draw_buffer_.get());

      // 逐像素拷贝：每隔 4 行 / 4 列取样，实现 1/4 缩小
      for (int ry = 0; ry < thumb_h; ++ry) {
        int sy = std::min(lh - 1, ry * 4);
        uint32_t* drow = dst32 + (dst_y + ry) * width_ + dst_x;
        const uint32_t* srow = lsrc + sy * lw;
        for (int rx = 0; rx < thumb_w; ++rx) {
          int sx = std::min(lw - 1, rx * 4);
          drow[rx] = srow[sx];
        }
      }
    }
