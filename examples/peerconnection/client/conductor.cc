...
#include "examples/peerconnection/client/image_seq_video_track_source.h"
...
// dummy video source that generates a solid color video frame that changes
class DummyVideoTrackSource : public webrtc::VideoTrackSource {
 public:
  static rtc::scoped_refptr<DummyVideoTrackSource> Create() {
    auto src = rtc::make_ref_counted<DummyVideoTrackSource>();
    src->Start();
    return src;
  }

  ~DummyVideoTrackSource() override { Stop(); }

 protected:
  DummyVideoTrackSource() : webrtc::VideoTrackSource(/*remote=*/false) {}
 ...
 private:
  void Start() {
    if (running_) return;
    running_ = true;
    thread_ = std::thread([this]() { GenerateLoop(); });
  }
  ...
  void GenerateLoop() {
    ...
    struct ColorYUV { uint8_t y, u, v; };
    std::vector<ColorYUV> colors = {
        {76, 85, 255},   // 红
        {150, 44, 21},   // 绿
        {29, 255, 107},  // 蓝
        {226, 0, 148},   // 黄
    ...
    while (running_) {
      ...
      memset(buffer->MutableDataY(), c.y, buffer->StrideY() * height);
      memset(buffer->MutableDataU(), c.u, buffer->StrideU() * height / 2);
      memset(buffer->MutableDataV(), c.v, buffer->StrideV() * height / 2);

      int64_t timestamp_us =
          start_time_us + frame_count * frame_interval_ms * 1000;
